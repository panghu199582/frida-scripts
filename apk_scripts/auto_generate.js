// Auto-Generate Hash Script
// Usage: frida -H 127.0.0.1:8888 -f com.telkom.mwallet -l auto_generate.js

Java.perform(function() {
    console.log("[*] 🚀 Initializing Auto-Generator...");

    // 1. Static Keys (Extracted)
    var HEX1 = "";
    var HEX2 = "";
    var HEX3 = "";

    // 2. Dynamic Params (Replace as needed)
    var DEV_ID = "c96b489bc518e58c";  // YOUR DEVICE ID
    var IMSI = "ff6805b16cbb1911";      // YOUR IMSI
    var VERSION = "4.43.0";

    // 3. Find Instance
    var ExternalFun = Java.use("module.libraries.coresec.ExternalFun");
    var instance = null;

    // Try finding existing instance
    Java.choose("module.libraries.coresec.ExternalFun", {
        onMatch: function(i) { instance = i; },
        onComplete: function() {}
    });
    
    // Create new if invalid
    if (!instance) {
        console.log("[*] Creating new ExternalFun instance...");
        instance = ExternalFun.$new();
    }

    // 4. GENERATE LOOP (Every 2 seconds)
    function generate() {
        try {
            // VERIFICATION MODE: Use fixed TS from Step 215
            var ts = "1767841753710"; 
            // var ts = Date.now().toString(); // Uncomment for live mode
            
            var tsLong = parseInt(ts);
            var saltedTs = ts + "tw4ll3tn30"; // Salt Logic
            
            var hash = instance.encryptHmacRaw(
                ts,  // Fix: Pass as String, not Long
                DEV_ID,
                IMSI,
                saltedTs,
                HEX1,
                HEX2,
                HEX3,
                VERSION
            );

            console.log("\n" + "=".repeat(50));
            console.log("⏰ Time: " + ts);
            console.log("🧂 Salt: " + saltedTs);
            console.log("🔑 Hash: " + hash);
            console.log("=".repeat(50));
            
        } catch(e) {
            console.log("[-] Error: " + e);
        }
    }

    // Run once immediately
    generate();
    
    // Suggest keeping alive or running periodically
    // setInterval(generate, 5000); 
    console.log("[*] Done. (Edit script to loop if needed)");
});
